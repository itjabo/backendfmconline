...
class C_site extends CI_Controller {
	public function getSite(){		
			...
			$data = $this->m_site->getSite($lac, $cellid);	
            $datas = array(
				'data_query'  	=> $data,
			);		
			$this->load->view('getSite',$datas);		
	}
	public function getNama(){		
			$jsonData = json_decode(trim(file_get_contents('php://input')), true);
			$username = $jsonData['username'];
			$this->load->model('m_site'); 
			$data = $this->m_site->getNama($username);	
			 $datas = array(
				'data_query'  	=> $data,
			);		
			$this->load->view('respon',$datas);	
	}
	...
	public function submit_laporan(){		
		$target_path = "./uploads/". str_replace( '-', '',date('Y-m-d'));
		$foto  =  str_replace( '-', '',date('Y-m-d')).'/'.$this->input->post('fileName');
		$foto2  =  str_replace( '-', '',date('Y-m-d')).'/'.$this->input->post('foto2');
		$foto3  =  str_replace( '-', '',date('Y-m-d')).'/'.$this->input->post('foto3');
		$foto4  =  str_replace( '-', '',date('Y-m-d')).'/'.$this->input->post('foto4');
		$sn2  = $this->input->post('sn2');
		$sn3  = $this->input->post('sn3');
		$sn4  = $this->input->post('sn4');
		$dl2  = $this->input->post('dl2');
		$dl3  = $this->input->post('dl3');
		$dl4  = $this->input->post('dl4');
		...
		$this->load->model('m_site'); 
		$this->m_site->insert_laporan($id_lap, $site_id, $username, $tanggal, $foto, $foto2, $foto3, $foto4, $sn2, $sn3, $sn4, $dl2, $dl3, $dl4, $catatan);
		$target_path = "./uploads/". str_replace( '-', '',date('Y-m-d'));
		...
		$datax = array(
			...
			$this->load->view('respon_submit',$datas);		
	...
	public function getLaporan(){		
			$this->load->model('m_site'); 
			$data = $this->m_site->getLaporan();	
            $datas = array(
				'data_query'  	=> $data,
			);		
			$this->load->view('respon',$datas);		
	}
	public function selectLap(){		
			$jsonData = json_decode(trim(file_get_contents('php://input')), true);
			$id_lap = $jsonData['id_lap'];
			$this->load->model('m_site'); 
			$data = $this->m_site->selectLap($id_lap);	
            $datas = array(
				'data_query'  	=> $data,
			);		
			$this->load->view('respon',$datas);		
	...
	public function getPeta()
	{
		...
		$this->load->model('m_site');
		$data = $this->m_site->getPeta($lat1, $lat2, $long1, $long2);
		$datas = array(
				'data_query'  	=> $data,
			);				
		$this->load->view('respon',$datas);		
